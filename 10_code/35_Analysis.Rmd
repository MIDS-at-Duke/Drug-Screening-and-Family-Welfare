...
p <- ggplot(data = us_states,
            aes(x = long, y = lat,
                group = group, fill = drug_law)) + geom_polygon(color = "white", size = 0.20, alpha = 0.75) + coord_map() + labs(x = '', y = '', title = 'State Drug Law Enaction') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name = 'Policy Status', labels = c('No', 'Yes'), values = c('lightblue', 'navy'))
p
```
...
pTN <- ggplot(data = us_states,
            aes(x = long, y = lat,
                group = group, fill = TNRegion)) + geom_polygon(color = "white", size = 0.20, alpha = 0.75) + coord_map() + labs(x = '', y = '', title = 'Tennessee and Control States') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name = 'State Groups', labels = c('Tennessee', 'Control States', 'Non-Control States'), values = c('tomato', 'yellow', 'lightgray'))
```
...
TNPost <- TN %>% filter(Date >= '2015-07-01')
ControlPost <- R1 %>% filter(Date >= '2015-07-01')
```

```{r, warning = F, message = F}
cols <- c("Tennessee" = "tomato", "Control Region" = "yellow")
RegInit <- ggplot() + stat_smooth(TNPre, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'Tennessee', group = 1),
  color = "darkred", size = 0.20,
  method = "lm") + stat_smooth(TNPost, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'Tennessee', group = 1),
  color = "darkred", size = 0.20, 
  method = "lm"
  ...
  method = "lm"
                ...
                color = "black", size=0.50) + geom_vline(xintercept = as.Date('2015-07-01'), linetype="dashed", 
                color = "black", size=0.50) + stat_smooth(ControlPost, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'Control Region', group = 1),
  color = "darkred", size = 0.20, 
  method = "lm"
  ) + labs(x = 'Date', y = 'Percent of Families on TANF', title = 'Linear Time Trends for Percent of Families on TANF: \n Tennessee and Control Region') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name="Policy Status",values=cols)
```
...
p <- ggplot(data = us_states,
            aes(x = long, y = lat,
                group = group, fill = NCRegion)) + geom_polygon(color = "white", size = 0.20, alpha = 0.75) + coord_map() + labs(x = '', y = '', title = 'North Carolina and Control States') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name = 'State Groups', labels = c('North Carolina', 'Control States', 'Non-Control States'), values = c('tomato', 'yellow', 'lightgray'))
```
...
RegInit <- ggplot() + stat_smooth(WVPre, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'North Carolina', group = 1),
  color = "darkred", size = 0.20,
  method = "lm") + stat_smooth(WVPost, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'North Carolina', group = 1),
  color = "darkred", size = 0.20, 
  method = "lm"
  ) + stat_smooth(ControlPre, mapping = aes(x = Date, y = Scaled_LogFam, fill = 'Control Region', group = 1),
  color = "darkred", size = 0.20,
