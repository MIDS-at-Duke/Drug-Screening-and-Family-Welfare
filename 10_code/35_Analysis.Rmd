...
final <- read.csv("E:/final_dataset_clean.txt")
```
...
pTN <- ggplot(data = us_states,
            aes(x = long, y = lat,
                group = group, fill = TNRegion)) + geom_polygon(color = "white", size = 0.20, alpha = 0.75) + coord_map() + labs(x = '', y = '', title = 'Tennessee and Control State of South Carolina') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name = 'State Group', labels = c('Tennessee', 'Control State: \n South Carolina', 'Non-Control States'), values = c('#364C54', '#BCD2CB', 'darkgray'))
```
...
cols <- c("Tennessee" = '#364C54', "Control State: South Carolina" = "#BCD2CB")
RegInit <- ggplot() + geom_line(TNPre, mapping = aes(x = Date, y = scale_tanf_fams, group = 1),
  color = "darkgray") + stat_smooth(TNPre, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Tennessee', group = 1),
  color = "darkgray", size = 0.20, alpha = 0.75, 
  method = "lm") + geom_line(TNPost, mapping = aes(x = Date, y = scale_tanf_fams, group = 1),
  color = "darkgray") + stat_smooth(TNPost, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Tennessee', group = 1),
  color = "darkgray", size = 0.20, alpha = 0.75,   
  method = "lm") + geom_line(ControlPre, mapping = aes(x = Date, y = scale_tanf_fams, group = 1),
  color = "darkgray") + stat_smooth(ControlPre, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Control State: South Carolina', group = 1),
  ...
  method = "lm"
                ...
                color = "black", size=0.50) + geom_line(ControlPost, mapping = aes(x = Date, y = scale_tanf_fams, group = 1),
  color = "darkgray") + stat_smooth(ControlPost, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Control State: South Carolina', group = 1),
  color = "darkgray", size = 0.20, 
  method = "lm"
  ) + labs(x = 'Date', y = 'Percent of Families on TANF', title = 'Linear Time Trends for Percent of Families on TANF: \n Tennessee and Control State') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name="State Group",values=cols)
```
...
StatesListTN <- c('Tennessee', 'South Carolina')
...
```{r}
All <- c('Michigan', 'Wisconsin', 'Illinois', 'Indiana', 'Ohio', 'Iowa', 'Minnesota', 'Pennsylvania', 'South Carolina', 'New York')
Test <- final %>% filter(state %in% All)

ggplot() + geom_line(data = Test, mapping= aes(x = Date, y = scale_tanf_fams, color = state))
```
...
Control Region: Ohio, Pennsylvania, New York.

```{r}
Region1Overall <- c('Ohio', 'Pennsylvania', 'New York')
R1 <- final %>% filter(state %in% Region1Overall) 

TN <- final %>% filter(state == 'Wisconsin') 

#Define pre and post time periods by breakpoint: 
TNPre <- TN %>% filter(Date <= '2015-11-01')
ControlPre <- R1 %>% filter(Date <= '2015-11-01')

TNPost <- TN %>% filter(Date >= '2016-01-01')
ControlPost <- R1 %>% filter(Date >= '2016-01-01')
```
...
```{r}
All <- c('Utah', 'New Mexico', 'Nevada', 'California', 'Idaho', 'Wyoming', 'Colorado', 'Arizona')
Test <- final %>% filter(state %in% All)
...
```{r}
#Visualize TN and Controls geographically: 
Region1Overall <- c('Colorado', 'New Mexico')
...
pUtah <- ggplot(data = us_states,
            aes(x = long, y = lat,
                group = group, fill = UtahRegion)) + geom_polygon(color = "white", size = 0.20, alpha = 0.75) + coord_map() + labs(x = '', y = '', title = 'Utah and Control State of New Mexico') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name = 'State Group', labels = c('Utah', 'Control State: \n New Mexico', 'Non-Control States'), values = c('#364C54', '#BCD2CB', 'darkgray'))
```

```{r}
Region1 <- c('New Mexico', 'Colorado', 'Nevada')
...
RegInit <- ggplot() + geom_line(WVPre, mapping = aes(x = Date, y = scale_tanf_fams), color = 'darkgray') + stat_smooth(WVPre, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Utah', group = 1),
  color = "darkgray", size = 0.20,
  method = "lm") + geom_line(WVPost, mapping = aes(x = Date, y = scale_tanf_fams), color = 'darkgray') + stat_smooth(WVPost, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Utah', group = 1),
  color = "darkgray", size = 0.20, 
  method = "lm"
  ) + geom_line(ControlPre, mapping = aes(x = Date, y = scale_tanf_fams), color = 'darkgray') + stat_smooth(ControlPre, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Control Region', group = 1),
  color = "darkgray", size = 0.20,
  method = "lm"
                ...
                color = "black", size=0.50) + geom_line(ControlPost, mapping = aes(x = Date, y = scale_tanf_fams), color = 'darkgray') + stat_smooth(ControlPost, mapping = aes(x = Date, y = scale_tanf_fams, fill = 'Control Region', group = 1),
  color = "darkgray", size = 0.20, 
  method = "lm"
  ) + labs(x = 'Date', y = 'Number of Families on TANF', title = 'Linear Time Trends for Number of Families on TANF: \n Utah and Control Region') + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + scale_fill_manual(name="State Group",values=cols)
```
