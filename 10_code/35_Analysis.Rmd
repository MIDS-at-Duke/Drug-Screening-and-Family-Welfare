...
```{r}
All <- c('Michigan', 'Wisconsin', 'Illinois', 'Indiana', 'Ohio', 'Iowa', 'Minnesota', 'Pennsylvania')
...
Control Region: Ohio, Pennsylvania.

```{r}
Region1Overall <- c('Illinois')
R1 <- final %>% filter(state %in% Region1Overall) 
...
```{r}
#Now, make a new dataframe with all relevant states: 
StatesListUtah <- c('Utah', 'New Mexico')
...
UtahData$state <- as.factor(TNData$state)
```

```{r, comment = ''}
#Fixed Effects DID model: 
UtahReg <- felm(scale_tanf_fams ~ Drug_Law + Post_Ind + scale_num_below_fpl + unemp_rate + Drug_Law*Post_Ind | year_cat, data = UtahData)
